...
# 1. Introduction
Languages have different morph syntactical means to express causal relations. From a syntactic point of view, there are polipredicative (Мы не гуляли, потому что шёл дождь) and nominal causal constructions (Мы не гуляли из-за дожди). This study focuses on nominal causal constructions in Basque. Depending on factors not yet elucidated, when expressing causal relations, Basque writers choose different case markers: causal, instrumental, genitive, benefactive, or destinative. Until present, no attempt has been undertaken to describe the encoding of causal relations in Basque. My intuition is that the choice of the case marker depends on the causal semantic role (Cause, Force, Motive). Of course, there may be other factors, such as dialectal and diachronic differences. Beyond an initial description and quantification of the encoding of causal roles in nominal constructions, I will formulate some basic questions.

# 2. Typological ground
Semantic roles Force and Cause. In Indo European languages Force is often coded as Instrument AND the instrumental expresses both Cause and Instrument [Luraghi 2010: 47]. Given that Basque is an isolated in terms of genetical relationships (albeit in a long lasting, intensive situation of contact with I-E languages), does Basque behave as an I-E language to this respect? Basque has both causal and instrumental cases. As an “I-E behavior” I expect Force to be coded by the instrumental case. As an “non-I-E behavior” I expect Force not to be coded by the instrumental case.
Semantic roles Cause and Reason [for which I prefer the term Motive]. Although from a cognitive point of view Cause and Reason are significantly different, at least in the Indo-European languages they are not formally distinct: apparently the normal situation is one where there is only one grammaticalized semantic role [Luraghi 2010: 54]. Does Basque behave as an I-E language to this respect or it encodes differently Cause and Reason? Basque has a causal case. As an “I-E behavior” I expect both Cause and Motive to be coded similarly. As an “non-I-E behavior” I expect Cause and Motive to be coded differently.
Future research should consider other semantic roles that overlap in the expression of causal relations (e.g. Purpose and Beneficiary).

# 3. 	Description of the data
NB: This project is conceived as the basis for a more extensive research that I will eventually conduct during the next year. Thus, in the case my data are not representative enough to prove or discard a hypothesis in terms of statistical significance, I will hopefully have the opportunity to solve the problem. 
I have compiled a small corpus of texts of historical Basque, some of them manually transcribed. The central part of the corpus consists of seven translations of the Gospel of Mark into four dialects, from 1571 to 2004. Five more texts have been added, in order to represent a given dialect at a given period. At the time being, the time span of this corpus covers from 1545 to 2004, and its size is 141,000 tokens.
Data consists of 2,338 observations. Each observation corresponds to one occurrence of any of the case-markers involved in the expression of the causal roles. For each observation I have annotated the following parameters: 1) number of verse, page, etc. where the example is found; 2) English translation --only for the versions of the Gospel--; 3) Basque example; 4) marker --suffix, postposition--; 5) allomorph, if any; 6) label of the case marker; 7) semantic role; 8) noun class --I have included interrogatives and connectors just for convenience--; 9) morphsyntactic remarks, if any; 10) semantic remarks, if any; 11) tittle of the text; 12) author; 13) year; 14) century; 15) place; and 16) dialect. 
...
Derived questions: If yes, are these differences observable in all dialects? Are they stable throughout the history of Basque?
We also can undertake the converse way, i.e. analyze the semantic roles expressed by the case markers (causal, instrumental, genitive, benefactive):
Which semantic roles does the causal (instrumental, genitive, benefactive) case-marker encode?
...
Are there differences in the encoding Cause, Motive, and Force in Basque?
H0: encoding (Cause) = encoding (Motive) = encoding (Force)
H1: there are differences between any two of the three encodings

Here we need to compare two categorical variables: which causal role is expressed by means of which case marker. For this purpose, the suitable statistical tool is a chi squared test, for which we need to use a table of contingency bigger than 2x2:at least 3 causal roles * 4 case-markers.
...
library(dplyr)
library(tidyverse)
# NB: Even though this step can be done more quickly and succinctly, I will do some intermediary steps that might be useful later.
...
# 2. The function balloonplot() draws a graphical matrix where each cell contains a dot whose size reflects the relative magnitude of the corresponding component.
...
```
Blue color in the mosaicplot indicates that the observed value is higher than the expected value if the data were random.
Conversely, red color specifies that the observed value is lower than the expected value if the data were random. 
This mosaic plot points to relevant differences between the encoding of Cause, Motive ad Force. The most remarkable are: 
1) the use of the genitive in the expression of Cause (indeed it hardly occur with the other roles); 
...
# Let us check the statistical significace of the remarks above.
```{r}
...
# As the workspace needed is large, in order not to waste the time of the reader, I desactivate the code in this chunk. Results obtained:
...
# 4.2 QUESTION 2. Are these differences of encoding observed in all dialects?
...
```
The two graphs point to relevant differences in the encoding of Cause between the Basque dialects. The most outstanding insights are:
...
4) the benefactive case is absolutely marginal in the expression of Cause.
...
1) The benefactive case is unexpectedly use in Labourdin and Souletin dialects (especially in Labourdin); and 
...
chisq
# As the workspace needed is large, in order not to waste the time of the reader, I desactivate the code in this chunk.
...
```
Even simulating p-values, the statistical tools employed give p-values considerably over 0.5, what means that at the usual confidance level of 95% the null hypothesis cannot be rejected. I.e., concerning the encoding of Force there are no significant differences between Basque dialects.
...
Alternative hypothesis: the encoding of Cause, Force, and Motive is not similar throughout the five centuries.
...
NS: The initial data frame had a column for "period", which for many specialits in diachrony is the most desirable parameter to compare data, but here I opt for working with years, for obvious reaons of usability in statistics.
...
ggplot(data = Cause_bin, aes(x = year, y = GEN)) +
  ...
  theme(legend.position = "none")
```
For the encoding of Cause, the logistic model does not show dependence between the use of case-markers and the year. Considering by case-marker, there is total independency between the causal case (CAUS) and year. The other two cases analyzed show complementary relations of dependency: 
1) for the instrumental case, the regression model points to an increasing in its use by year (coefficient 0.001463);
2) for the genitive case, the dependency is defined in terms of negative coefficient (-0.0041), which means that every year the probability of GEN for the expression of Cause decrease. 
Obviously, the values for these coefficiennts would be much higher if we considered centuries instead of years.

```{r}
# Здесь я пытался визуализировать вероятность родительного падежа для выражения причины в завизимости от года, но кое-что не прлучилось.
...
# In terms of linguistic evolution, the encoding of Motive by means of the benefactive case is an innovation. 
...
ggplot(data = Force_bin, aes(x = year, y = INS)) +
  ...
  theme(legend.position = "none")
```
Finally, and not surprisingly, the number of observations for the role Force is too small to obtain significant results. The picture is simple: this roles is encoded by two case-markers, and the two show complementary evolution: while the use of the causal case gets increased in historical records, the use of the instrumental decreases to a similar extent. 
